...
export function PlayerHero({labels}){
 ...
 return <section className={'hero-player'} id={'inicio'}><div className={'hero-copy'}><p className={'eyebrow'}>{labels.welcome}</p><h1>{labels.title}</h1><p className={'description'}>{labels.description}</p><div className={'bulletin-card'} id={'boletin'}><span>Audio</span><div><p>{labels.bulletinTitle}</p><small>{labels.bulletinMeta}</small></div></div><div className={'player-controls'}><button className={'play-button'} onClick={play} type={'button'}>{on?labels.pause:labels.play}</button><span className={'audio-status'}>{on?labels.playing:labels.ready}</span></div></div><div className={'hero-visual'}><div className={'radio-mark'}>RH</div><p>{labels.status}</p></div></section>
}
